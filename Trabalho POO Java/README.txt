...
Modelando Veículos com Herança
Objetivo: Este exercício visa reforçar o conceito de herança em programação orientada a
objetos, demonstrando como classes derivadas podem herdar e especializar características de
uma classe base.
Contexto: Vamos modelar diferentes tipos de veículos. Todos os veículos compartilham
algumas características básicas, mas cada tipo tem suas particularidades.
Instruções:
1. Crie uma classe base chamada Veiculo:
o Esta classe deve ter os seguintes atributos privados:
...
 ano (int)
 velocidadeAtual (double)
o Implemente um construtor público que receba o modelo e o ano como
parâmetros e inicialize a velocidadeAtual com 0.0.
o Implemente os seguintes métodos públicos:
 getModelo(): Retorna o modelo do veículo.
 getAno(): Retorna o ano do veículo.
 getVelocidadeAtual(): Retorna a velocidade atual do veículo.
 acelerar(double incremento): Aumenta a velocidadeAtual pelo valor
do incremento. Se o incremento for negativo, exiba uma mensagem
de erro e não altere a velocidade.
 frear(double decremento): Diminui a velocidadeAtual pelo valor do
decremento. Se o decremento for negativo ou maior que a
velocidadeAtual, exiba uma mensagem de erro e não altere a
velocidade.
 exibirInformacoes(): Imprime o modelo, o ano e a velocidade atual do
veículo.
2. Crie duas classes derivadas de Veiculo:
o Carro:
 Adicione um atributo privado específico: numeroPortas (int).
 Implemente um construtor público que receba o modelo, o ano e o
numeroPortas como parâmetros. Lembre-se de chamar o construtor
da superclasse (Veiculo).
 Implemente um método público específico: abrirPorta(): Imprime uma
mensagem indicando que uma porta do carro foi aberta.
 Sobrescreva o método exibirInformacoes() para incluir o número de
portas nas informações exibidas.
o Moto:
 Adicione um atributo privado específico: cilindradas (int).
 Implemente um construtor público que receba o modelo, o ano e as
cilindradas como parâmetros. Lembre-se de chamar o construtor da
superclasse (Veiculo).
 Implemente um método público específico: empinar(): Imprime uma
mensagem indicando que a moto está empinando (apenas para
demonstração!).
 Sobrescreva o método exibirInformacoes() para incluir as cilindradas
nas informações exibidas.
3. Crie uma classe de teste chamada TestaVeiculos com o método main():
o Dentro do método main(), realize as seguintes ações:
 Crie um objeto da classe Carro com modelo "Sedan", ano 2023 e 4
portas.
 Crie um objeto da classe Moto com modelo "Esportiva", ano 2024 e
600 cilindradas.
 Exiba as informações iniciais de ambos os veículos usando o método
exibirInformacoes().
 Acelere o carro em 30 km/h e a moto em 50 km/h.
 Exiba novamente as informações de ambos os veículos.
 Tente frear o carro em 50 km/h e a moto em 20 km/h.
 Exiba mais uma vez as informações.
 Chame os métodos específicos abrirPorta() no objeto Carro e
empinar() no objeto Moto.

4. Tratamento de Exceções (Existente):
o Modifique os métodos acelerar() e frear() na classe Veiculo para lançarem uma
IllegalArgumentException caso o incremento ou decremento sejam negativos
(para acelerar) ou se o decremento for maior que a velocidadeAtual (para
frear).
o Na classe TestaVeiculos, envolva as chamadas aos métodos acelerar() e frear()
em blocos try-catch para capturar e exibir a mensagem da
IllegalArgumentException caso ela ocorra.
5. Tratamento de Exceções (Nova Classe Personalizada):
o Crie uma nova classe de exceção personalizada chamada AnoInvalidoException
que estenda a classe Exception. Esta classe deve ter um construtor que recebe
uma mensagem de erro.
o Modifique o construtor da classe Veiculo para lançar uma
AnoInvalidoException se o ano fornecido for menor que 1900.
o Na classe TestaVeiculos, envolva a criação dos objetos Carro e Moto em blocos
try-catch para capturar e exibir a mensagem da AnoInvalidoException caso ela
seja lançada durante a criação de um veículo com um ano inválido.
Tratamento de Exceção Pretendido (Adicional):
Além do tratamento de IllegalArgumentException para aceleração e frenagem, pretendo que
criemos uma exceção personalizada (AnoInvalidoException) para lidar com a criação de
veículos com anos considerados inválidos (anteriores a 1900). Isso demonstrará como criar e
tratar nossas próprias exceções para cenários específicos da aplicação, tornando o código mais
claro e permitindo um tratamento mais direcionado para diferentes tipos de erros. A classe de
teste (TestaVeiculos) capturará essa exceção durante a instanciação dos objetos veículo.